...
@section('content')
  <div class="card" style="max-width:1250px">
    <h1 style="margin:0 0 14px 0;">Edit System</h1>

    <form method="post" action="{{ route('admin.star-systems.update', $item) }}">
      @csrf
      @method('PUT')
      ...
      <div style="display:grid;grid-template-columns:.95fr 1.05fr;gap:18px;align-items:start;">
        <div>
          <div class="grid grid-2">
            <div class="field">
              ...
              <select name="galaxy_id" required>
                @foreach($galaxies as $g)
                  <option value="{{ $g->id }}" @selected(old('galaxy_id', $item->galaxy_id) == $g->id)>
                    #{{ $g->id }} {{ $g->name }}
                  </option>
                @endforeach
              </select>
            ...
            <div class="field">
              <label>X</label>
              <input type="number" step="0.01" name="x" value="{{ old('x', $item->x) }}" required>
            </div>

            <div class="field">
              <label>Y</label>
              <input type="number" step="0.01" name="y" value="{{ old('y', $item->y) }}" required>
            </div>

            <div class="field">
              <label>Z</label>
              <input type="number" step="0.01" name="z" value="{{ old('z', $item->z) }}" required>
            </div>

            <div class="field">
              <label>Select Sun</label>
              <select id="sunTypeInput" name="color_hex" required>
                <option value="#ffdd99" @selected(old('color_hex', $item->color_hex ?? '#ffdd99') == '#ffdd99')>Yellow</option>
                <option value="#ff5555" @selected(old('color_hex', $item->color_hex ?? '#ffdd99') == '#ff5555')>Red</option>
                <option value="#66aaff" @selected(old('color_hex', $item->color_hex ?? '#ffdd99') == '#66aaff')>Blue</option>
              </select>
            </div>

            <div class="field">
              <label>Temperature</label>
              <input type="number" name="temperature" value="{{ old('temperature', $item->temperature ?? 5800) }}" required>
            </div>

            <div class="field">
              <label>Base Scale</label>
              <input id="baseScaleInput" type="number" step="0.01" name="base_scale" value="{{ old('base_scale', $item->base_scale ?? 1.0) }}" required>
            </div>
          </div>
