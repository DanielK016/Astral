@extends('layouts.app')
@section('title','Create Planet')

@section('content')
  <script type="module" src="https://unpkg.com/@google/model-viewer/dist/model-viewer.min.js"></script>

  <div class="card" style="max-width:1180px">
    <h1 style="margin:0 0 14px 0;">Create Planet</h1>
        ...
        <div>
          <div class="grid grid-2">
            <div class="field">
              ...
              <select name="star_system_id" required>
                @foreach($systems as $s)
                  <option value="{{ $s->id }}" @selected(old('star_system_id') == $s->id)>
                    #{{ $s->id }} {{ $s->name }}
                  </option>
                @endforeach
              </select>
            ...
            <div class="field">
              ...
              <select name="type" id="planetType" required>
                @foreach(['biolum','desert','gas','ice','ocean','terran','vulcanic'] as $t)
                  <option value="{{ $t }}" @selected(old('type', 'terran') == $t)>{{ ucfirst($t) }}</option>
                @endforeach
              </select>
            </div>
            ...
            <div class="field">
              ...
              <input type="number" step="0.1" name="orbit_radius" value="{{ old('orbit_radius', 10) }}" required>
            </div>

            <div class="field">
              <label>Radius</label>
              <input type="number" step="0.01" name="radius" value="{{ old('radius', 1.0) }}" required>
            </div>

            <div class="field">
              <label>Rotation speed</label>
              <input type="number" step="0.0001" name="rotation_speed" value="{{ old('rotation_speed', 0.01) }}" required>
            </div>

            <div class="field">
              <label>Axial tilt</label>
              <input type="number" step="0.01" name="axial_tilt" value="{{ old('axial_tilt', 0) }}">
            </div>

            <div class="field">
              <label>Has rings</label>
              <select name="has_rings">
                <option value="0" @selected((string) old('has_rings', '0') === '0')>No</option>
                <option value="1" @selected((string) old('has_rings', '0') === '1')>Yes</option>
              </select>
            </div>

            <div class="field" style="grid-column:1/-1">
              <label>Meta JSON (optional)</label>
              <textarea name="meta_json">{{ old('meta_json', '') }}</textarea>
        ...
        <div>
          <div class="card" style="padding:14px;border:1px solid rgba(255,255,255,.12);border-radius:16px;background:rgba(255,255,255,.03);">
            <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:10px;">
              ...
              <span id="planetPreviewLabel" class="muted">terran</span>
            </div>
